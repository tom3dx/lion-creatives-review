...
(function () {
  ...
  // §12 progressive enhancement: mark JS as available immediately.
  ...
  function loader() {
    ...
    if (reduced) {
      ...
      document.body.classList.add('ready'); return;
    }
    ...
    function finish() {
      ...
      setTimeout(function () {
        el.setAttribute('data-done', 'true');
        // leave the ACCESSIBILITY tree too — clip-path alone keeps the
        // permanent 'Loading' status exposed to screen readers
        el.setAttribute('aria-hidden', 'true');
  ...
  function menu() {
    ...
    function setOpen(state) {
      ...
      // §11: a closed menu must be gone for keyboard and assistive tech,
      // not merely clipped out of sight.
      if (state) {
  ...
  function reveals() {
    // §23 inversion: the page is fully composed BEFORE this runs. JS arms the
    // clip just-in-time and the observer removes it — so no-JS, crashed-JS,
    // reduced-motion and interrupted states all show finished composition.
    // Safety layers that actually exist: the IntersectionObserver, the
    // scroll/resize sweep below, print rules that force everything open, and
    // a reduced-motion block that out-specifies the armed state.
    var items = document.querySelectorAll('.rv, .ap-plane, .reveal-type');
    if (!items.length) return;
    if (reduced || !('IntersectionObserver' in window)) return; // stay composed
    ...
    items.forEach(function (n) {
      // §25: only MEDIA and hero type earn reveal choreography. Text, lists
      // and structure are never gated, so no scroll position, capture, or
      // interrupted animation can make a region read as missing content.
    ...
    // belt-and-braces: if the observer ever misses (throttled tab, exotic
    // engine), a plain scroll check reveals anything that has entered view.
    function sweep() {
      var vh2 = window.innerHeight || document.documentElement.clientHeight;
      for (var i = pending.length - 1; i >= 0; i--) {
        var n = pending[i], r = n.getBoundingClientRect();
        if (r.top < vh2 * 0.96 && r.bottom > 0) {
          n.classList.add('revealed'); io.unobserve(n); pending.splice(i, 1);
        }
      }
      if (!pending.length) {
    ...
    function onSweep() {
      ...
      requestAnimationFrame(function () { tick = false; sweep(); });
    }
    window.addEventListener('scroll', onSweep, { passive: true });
    window.addEventListener('resize', onSweep);
  }

  /* ---------- 4b. HERO READOUT + FOOTER ASSEMBLY ---------- */
  function readout() {
    var el = document.querySelector('[data-readout-live]');
    if (!el || reduced) return;
    var fps = 0, frames = 0, last = performance.now();
    function loop(t) {
      frames++;
      if (t - last >= 1000) { fps = frames; frames = 0; last = t; el.textContent = 'RENDER ' + fps + ' FPS'; }
  ...
  function footerMark() {
    var m = document.querySelector('.fmark');
    if (!m || reduced || !('IntersectionObserver' in window)) return; // stays assembled
    m.classList.add('scattered'); // JS-armed: default state is assembled
    var io = new IntersectionObserver(function (entries) {
      entries.forEach(function (en) {
        if (en.isIntersecting) { m.classList.remove('scattered'); io.disconnect(); }
      });
    }, { threshold: 0.35 });
    io.observe(m);
  }
  ...
  function heroDuality() {
    ...
    // tolerate either hero shell (.hero in v1/v2, .herov3 in v3) and degrade safely
    var host = cv.closest('.hero, .herov3') || cv.parentElement;
    if (!host) return;
  ...
  function enquiry() {
    ...
    function show(i) {
      ...
      // §13: only the ACTIVE step is aria-current; completed steps get their
      // own state; upcoming steps carry neither.
  ...
  /* ---------- 10. REAL VIDEO — lazy, muted, IO-gated, reduced-motion aware ---------- */
  function video() {
